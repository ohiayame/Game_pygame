...
while running:
    for j, rect in enumerate(buttons):
                        ...
                        screen.blit(number_texts[j], (rect.x + (button_size - number_texts[j].get_width()) // 2,
                                                        rect.y + (button_size - number_texts[j].get_height()) // 2))
            ...
            for i, rect in enumerate(buttons):
                ...
                if rect.collidepoint(event.pos) and not selected[i]:
                    ...
                    if new_bingo_count > Bingo_count:
                        ...
                        if Bingo_count >= 3:
                            text = font.render(f"{new_bingo_count} BINGOS! Game Over in {game_count} moves.", True, RED)
                            game_over = True
                        elif Bingo_count == 1:
